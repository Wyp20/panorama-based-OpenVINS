...
bool DynamicInitializer::initialize(double &timestamp, Eigen::MatrixXd &covariance, std::vector<std::shared_ptr<ov_type::Type>> &order,
  ...
  //    bearing << feat->uvs_norm.at(cam_id).at(idx0)(0), feat->uvs_norm.at(cam_id).at(idx0)(1), feat->uvs_norm.at(cam_id).at(idx0)(2);
  ...
  for (auto const &feat : features) {
    ...
    for (auto const &camtime : feat.second->timestamps) {
      ...
      for (size_t i = 0; i < camtime.second.size(); i++) {
        ...
        uv_norm << (double)feat.second->uvs_norm.at(cam_id).at(i)(0), (double)feat.second->uvs_norm.at(cam_id).at(i)(1),(double)feat.second->uvs_norm.at(cam_id).at(i)(2);
